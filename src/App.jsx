...
export default function App() {
  ...
  // --- HISTORY TRAP (BACK BUTTON PROTECTION) ---
  useEffect(() => {
  ...
  const handleSubmit = () => {
    ...
    // CRITICAL: We removed the extra pushState here to prevent stack buildup.
    setView('result');
  };

  // --- CRITICAL FIX: FLICKER-FREE EXIT LOGIC ---
  const handleExitToWelcome = (fromBackPress = false) => {
    ...
    // 3. FLICKER FIX: Update View *IMMEDIATELY*
    // By switching the view first, React removes the Test/Result component 
    // and its 'popstate' listener instantly.
    setView('welcome');

    // 4. History Cleanup (Background)
    // If user clicked a button (Software Exit), we still need to pop the "Trap".
    if (!fromBackPress) {
        // Use a tiny timeout to let the render cycle finish.
        // Since the listener is already removed (step 3), this history.back()
        // will happen silently without triggering any traps or flickers.
        setTimeout(() => {
            window.history.back();
            isProgrammaticExit.current = false;
        }, 10);
    } else {
        // If user pressed Device Back, the stack is already popped.
        isProgrammaticExit.current = false;
    }
  };
        ...
        <div className="flex flex-1 overflow-hidden relative">
            ...
            `}>
                ...
                <div className="p-4 border-t border-gray-200 bg-gray-50 text-xs font-medium space-y-2">
                    ...
                    {/* UI FIX: Current Indicator is now a small empty ring */}
