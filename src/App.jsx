...
export default function App() {
  ...
  // 2. Settings (Simplified defaults)
  const [appName] = useState("DANIEL'S ANATOMY CBT APP");
  const [testTitle] = useState("ANAT 213: GENERAL EMBRYO AND GENETICS");
  const [testDuration] = useState(20);
  const [marksPerQuestion] = useState(2);
  ...
  useEffect(() => {
    const handleBackButton = (event) => {
      if (view === 'test') {
        ...
        const confirmLeave = window.confirm("Are you sure you want to quit the test? Your progress will be lost.");
        
        if (confirmLeave) {
          handleExit(); 
        } else {
          // Push state back to undo the back action
          window.history.pushState(null, '', window.location.href);
        }
      }
    };

    // If entering test mode, push a state so we can catch the back button
    if (view === 'test') {
      window.history.pushState(null, '', window.location.href);
