...
export default function App() {
  ...
  // --- ROBUST BACK BUTTON HANDLING (FIXED) ---
  useEffect(() => {
  ...
  // --- CRITICAL FIX: HISTORY UNWIND LOGIC ---
  const handleExitToWelcome = () => {
    ...
    // 3. Unwind Browser History to remove "traps"
    // If in result, we likely have [Welcome -> Test -> Result] (stack size +2)
    // If in test, we have [Welcome -> Test] (stack size +1)
    if (view === 'result') {
      // Go back 2 steps (Test push + Result push)
      window.history.go(-2);
    } else if (view === 'test') {
      // Go back 1 step (Test push)
      window.history.back();
    }

    // 4. Fallback & View Reset
    // We use setTimeout to ensure the history action processes before we visually switch.
    // This also acts as a fallback if the history stack wasn't deep enough.
    setTimeout(() => {
        setView('welcome');
      ...
      <div className="min-h-screen bg-gray-50 flex items-center justify-center p-4 font-sans text-gray-900">
        <div className="bg-white p-8 rounded-2xl shadow-xl max-w-md w-full text-center border border-blue-100 relative">
          ...
          <div className="mb-6 flex flex-col items-center justify-center gap-3">
            ...
            {/* UI REMOVAL: STATUS READY BADGE REMOVED HERE */}
          </div>
        ...
        <div className="flex flex-1 overflow-hidden relative">
            ...
            `}>
                ...
                <div className="p-4 border-t border-gray-200 bg-gray-50 text-xs font-medium space-y-2">
                    ...
                    {/* UI ADDITION: NEW CURRENT LEGEND ITEM */}
