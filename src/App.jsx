...
export default function App() {
  ...
  const handleSubmit = () => {
    ...
    // CRITICAL: We removed the extra pushState here.
    // 'Result' view now shares the same history state as 'Test'.
    // This reduces stack depth, making "Back to Home" work with just one pop.
    
    setView('result');
  };

  // --- CRITICAL FIX: SMART EXIT LOGIC ---
  const handleExitToWelcome = (fromBackPress = false) => {
    ...
    // 3. History Logic
    // If user clicked a button (Software Exit), we must manually pop the history trap.
    if (!fromBackPress) {
        window.history.back();
    }
    // If user pressed Device Back (Hardware Exit), the trap is ALREADY popped. 
