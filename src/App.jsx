...
import { 
  Play, Clock, CheckCircle, XCircle, ChevronLeft, ChevronRight, 
  RotateCcw, FileText, Lock, Upload, Trash2, Edit2, LogOut, X, 
  Settings, User, LayoutGrid, Type, Menu, Award, BarChart3, WifiOff
} from 'lucide-react';
...
import { 
  ...
  onSnapshot, deleteDoc, updateDoc, query, orderBy 
} from "firebase/firestore";

// --- YOUR FIREBASE CONFIGURATION (From your previously uploaded file) ---
const firebaseConfig = {
  ...
  messagingSenderId: "728041000000",
  appId: "1:728041...:web:..." 
};
...
export default function App() {
  ...
  const [settings, setSettings] = useState({
    appName: "DANIEL'S ANATOMY CBT APP",
  ...
  // --- 1. INITIALIZATION & DATABASE LISTENERS ---
  useEffect(() => {
    // 1. Generate Guest ID
    setGuestId("GST-" + Math.floor(1000 + Math.random() * 9000));
    ...
    const unsubQuestions = onSnapshot(qQuery, (snapshot) => {
      ...
      // If DB empty, show initial (or use CSV upload to populate)
      if (qList.length > 0) setQuestions(qList);
      else setQuestions(INITIAL_QUESTIONS); 
    });

    // 4. Listen to Results (Real-time admin dashboard)
    const rQuery = query(collection(db, "results"), orderBy("date", "desc"));
    const unsubResults = onSnapshot(rQuery, (snapshot) => {
      const rList = snapshot.docs.map(doc => ({ id: doc.id, ...doc.data() }));
      setTestResults(rList);
    });
  ...
  const startTest = () => {
    if (questions.length === 0) {
      alert("No questions loaded. Please wait or contact admin.");
      return;
    }
    setAnswers({});
    setScore(0);
  ...
  // Submit Test (Sends result to Firebase)
  const handleSubmit = async () => {
    ...
    const finalScore = rawScore * settings.marksPerQuestion;
    ...
    try {
      ...
      alert("Note: Result saved locally (Network error)");
    }
  ...
  // --- ADMIN SETTINGS UPDATES (Writes to Firebase) ---
  
  const updateSettings = async (newSettings) => {
    setSettings(newSettings); // Optimistic update
    await setDoc(doc(db, "global", "settings"), newSettings);
  };

  // --- ADMIN QUESTION MANAGEMENT (Writes to Firebase) ---

  const handleDeleteQuestion = async (id) => {
    if(window.confirm("Delete this question globally?")) {
  ...
  const handleClearResults = async () => {
    if(window.confirm("Clear ALL student results from database? This cannot be undone.")) {
      // Batch delete
      testResults.forEach(async (res) => {
  ...
  const handleCSVUpload = (e) => {
    ...
    reader.onload = async (event) => {
      ...
      alert(`Uploaded ${count} questions to the database!`);
    };
  ...
  if (loading) return <div className="h-screen flex items-center justify-center text-blue-600 font-bold">Connecting to Server...</div>;
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
      <div className="bg-white rounded-xl p-6 w-full max-w-lg shadow-2xl max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={handleSaveEdit} className="space-y-4">
          ...
          <button type="submit" className="w-full bg-blue-600 text-white py-2 rounded font-bold">Update in Database</button>
        </form>
      </div>
    </div>
  );
        ...
        <div className="max-w-4xl mx-auto space-y-6">
          <div className="flex justify-between items-center bg-white p-4 rounded-xl shadow-sm">
            <h1 className="text-xl md:text-2xl font-bold flex items-center">
              <Lock className="w-6 h-6 mr-2 text-blue-600"/> Admin Panel (Online)
            </h1>
            <button 
          ...
          <div className="bg-white p-6 rounded-xl shadow-sm border border-blue-100">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div>
                ...
                <input 
                  type="text" 
                  value={settings.testTitle}
                  onChange={(e) => updateSettings({...settings, testTitle: e.target.value})}
                  className="w-full border rounded-lg p-2"
                />
              </div>
              <div>
                ...
                <div className="flex items-center gap-4">
                  <button onClick={() => updateSettings({...settings, duration: settings.duration - 1})} className="p-2 bg-gray-100 rounded">-</button>
                  <span className="font-bold">{settings.duration}</span>
                  <button onClick={() => updateSettings({...settings, duration: settings.duration + 1})} className="p-2 bg-gray-100 rounded">+</button>
                </div>
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Marks/Question</label>
                <div className="flex items-center gap-4">
                  <button onClick={() => updateSettings({...settings, marksPerQuestion: settings.marksPerQuestion - 1})} className="p-2 bg-gray-100 rounded">-</button>
                  <span className="font-bold">{settings.marksPerQuestion}</span>
                  <button onClick={() => updateSettings({...settings, marksPerQuestion: settings.marksPerQuestion + 1})} className="p-2 bg-gray-100 rounded">+</button>
                </div>
              </div>
            </div>
          ...
          <div className="bg-white p-6 rounded-xl shadow-sm border border-purple-100">
             <div className="flex justify-between items-center mb-4">
                <h2 className="text-lg font-bold flex items-center text-gray-800">
                  <BarChart3 className="w-5 h-5 mr-2"/> Live Results ({testResults.length})
                </h2>
                <button onClick={handleClearResults} className="text-red-500 text-sm font-bold hover:underline">Clear History</button>
             </div>
                 ...
                 <tbody className="divide-y">
                   {testResults.map((res) => (
                     <tr key={res.id} className="hover:bg-gray-50">
                       <td className="p-3 font-mono font-bold text-blue-600">{res.guestId}</td>
                       <td className="p-3 font-bold">{res.score}/{res.total}</td>
                       <td className={`p-3 font-bold ${res.percentage >= 50 ? 'text-green-600' : 'text-red-600'}`}>{res.percentage}%</td>
          ...
          <div className="bg-white p-4 rounded-xl shadow-sm flex flex-col md:flex-row gap-4 items-center">
            ...
            <button onClick={() => fileInputRef.current.click()} className="flex items-center bg-blue-600 text-white px-4 py-2 rounded-lg font-bold">
              <Upload className="w-5 h-5 mr-2"/> Upload Questions (CSV)
            </button>
          </div>
          ...
          <div className="bg-white rounded-xl shadow-sm overflow-x-auto">
            <table className="w-full text-left text-sm min-w-[600px]">
              ...
              <tbody className="divide-y">
                {questions.map((q, idx) => (
                  <tr key={q.id || idx} className="hover:bg-gray-50">
                    ...
                    <td className="p-4 text-right">
                      <button onClick={() => setEditingQuestion(q)} className="text-blue-500 hover:bg-blue-50 p-2 rounded mr-2"><Edit2 className="w-4 h-4"/></button>
                      <button onClick={() => handleDeleteQuestion(q.id)} className="text-red-500 hover:bg-red-50 p-2 rounded"><Trash2 className="w-4 h-4"/></button>
                    </td>
                  </tr>
                ))}
              </tbody>
      ...
      <div className="min-h-screen bg-gray-50 flex items-center justify-center p-4 font-sans text-gray-900">
        <div className="bg-white p-8 rounded-2xl shadow-xl max-w-md w-full text-center border border-blue-100">
          <div className="mb-6 flex justify-center">
            <div className="bg-blue-100 p-4 rounded-full"><FileText className="w-12 h-12 text-blue-600" /></div>
          </div>
          <h1 className="text-2xl md:text-3xl font-bold text-gray-900 mb-1 uppercase tracking-tight">{settings.appName}</h1>
          <p className="text-gray-500 font-medium mb-6 uppercase text-xs md:text-sm">{settings.testTitle}</p>
          ...
          <div className="text-left bg-gray-50 p-4 rounded-lg text-sm text-gray-700 mb-8 space-y-2">
            <p className="flex items-center"><Clock className="w-4 h-4 mr-2"/> <strong>Time:</strong> {settings.duration} Mins</p>
            <p className="flex items-center"><FileText className="w-4 h-4 mr-2"/> <strong>Questions:</strong> {questions.length}</p>
            <p className="flex items-center"><CheckCircle className="w-4 h-4 mr-2"/> <strong>Marks:</strong> {settings.marksPerQuestion}/Q</p>
          </div>

          <button onClick={startTest} className="w-full bg-blue-600 hover:bg-blue-700 text-white font-bold py-4 rounded-xl shadow-lg flex items-center justify-center mb-6">
          ...
          <div className="border-t pt-6">
            ...
            <form onSubmit={handleAdminLogin} className="flex gap-2">
              <input type="password" placeholder="Password" value={adminPassInput} onChange={(e) => setAdminPassInput(e.target.value)} className="flex-1 border rounded px-3 py-2 text-sm" />
              <button className="bg-gray-800 text-white px-3 py-2 rounded text-sm font-bold">Login</button>
            </form>
          </div>
        </div>
      </div>
      ...
      <div className="h-screen bg-gray-50 flex flex-col font-sans overflow-hidden">
        <header className="bg-white shadow-sm px-3 md:px-6 py-3 flex justify-between items-center z-20 flex-shrink-0 w-full relative">
          <div className="flex items-center gap-2 overflow-hidden mr-2">
            <button onClick={() => setIsMobileMenuOpen(!isMobileMenuOpen)} className="md:hidden p-2 hover:bg-gray-100 rounded-lg flex-shrink-0"><Menu className="w-5 h-5 text-gray-600"/></button>
            <div className="flex flex-col overflow-hidden min-w-0">
              <h1 className="font-bold text-gray-800 text-xs md:text-lg uppercase truncate">{settings.testTitle}</h1>
          ...
          <div className="flex items-center gap-2 md:gap-4 flex-shrink-0">
            <div className={`font-mono font-bold text-sm md:text-xl ${timeLeft < 60 ? 'text-red-600 animate-pulse' : 'text-blue-600'} flex items-center`}>
              <Clock className="w-4 h-4 md:w-5 md:h-5 mr-1 md:mr-2" /> {formatTime(timeLeft)}
            </div>
            <button onClick={() => {if(window.confirm("Submit?")) handleSubmit();}} className="bg-red-600 text-white px-3 py-1.5 md:px-4 md:py-2 rounded-lg font-bold text-xs md:text-base">Submit</button>
          </div>
        </header>

        <div className="flex flex-1 overflow-hidden relative">
            {isMobileMenuOpen && <div className="absolute inset-0 bg-black bg-opacity-50 z-30 md:hidden" onClick={() => setIsMobileMenuOpen(false)} />}
            <aside className={`absolute inset-y-0 left-0 z-40 w-64 bg-white border-r shadow-xl transform transition-transform duration-300 md:relative md:translate-x-0 md:shadow-none md:z-10 md:w-72 ${isMobileMenuOpen ? 'translate-x-0' : '-translate-x-full'}`}>
                <div className="p-4 border-b bg-gray-50 flex justify-between items-center">
                    <h2 className="font-bold text-gray-700 flex items-center text-sm md:text-base"><LayoutGrid className="w-4 h-4 md:w-5 md:h-5 mr-2 text-blue-600"/> Navigator</h2>
                    <button onClick={() => setIsMobileMenuOpen(false)} className="md:hidden p-1 text-gray-500"><X className="w-5 h-5"/></button>
                </div>
                <div className="flex-1 overflow-y-auto p-4">
                    <div className="grid grid-cols-5 gap-2">
                        {questions.map((q, idx) => {
                            ...
                            return (
                               <button key={q.id || idx} onClick={() => { setCurrentQIndex(idx); setIsMobileMenuOpen(false); }} className={`h-8 w-8 md:h-10 md:w-10 rounded-lg text-xs md:text-sm font-bold border ${isCurrent ? 'ring-2 ring-blue-600 border-blue-600' : ''} ${isAnswered ? 'bg-green-100 text-green-700' : 'bg-gray-50'}`}>{idx + 1}</button>
                            );
                        })}
                    </div>
                </div>
                ...
                <div className="max-w-3xl mx-auto w-full">
                    <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-4 md:p-10 min-h-[50vh]">
                        <div className="mb-4 md:mb-6 flex justify-between items-center">
                            <span className="bg-blue-100 text-blue-800 px-2 py-1 md:px-3 rounded-full text-[10px] md:text-xs font-bold uppercase tracking-wider">Question {currentQIndex + 1} of {questions.length}</span>
                            <span className="md:hidden text-[10px] font-bold text-gray-400">{Object.keys(answers).length}/{questions.length} Done</span>
                        </div>
                        <h2 className="text-lg md:text-2xl font-medium text-gray-800 mb-6 md:mb-8 leading-relaxed break-words">{question ? question.text : "Loading..."}</h2>
                        <div className="space-y-3">
                            {['optionA', 'optionB', 'optionC', 'optionD'].map((optKey) => (
                                <button key={optKey} onClick={() => handleAnswerSelect(question.id, optKey)} className={`w-full text-left p-3 md:p-4 rounded-xl border-2 flex items-start md:items-center ${answers[question.id] === optKey ? 'border-blue-500 bg-blue-50 text-blue-900' : 'border-gray-100 hover:border-blue-200'}`}>
                                <div className={`w-5 h-5 md:w-6 md:h-6 rounded-full border-2 mr-3 flex-shrink-0 ${answers[question.id] === optKey ? 'border-blue-500' : 'border-gray-300'}`}>{answers[question.id] === optKey && <div className="w-full h-full rounded-full bg-blue-500 scale-50" />}</div>
                                <span className="text-sm md:text-base text-gray-700 font-medium break-words"><span className="font-bold mr-2 uppercase text-xs md:text-sm">{optKey.replace('option', '')}.</span>{question ? question[optKey] : ""}</span>
                                </button>
                            ))}
                        </div>
                        <div className="flex justify-between mt-8 md:mt-10 pt-6 border-t border-gray-100">
                            <button disabled={currentQIndex === 0} onClick={() => setCurrentQIndex(prev => prev - 1)} className="flex items-center text-gray-500 hover:text-blue-600 disabled:opacity-30 font-medium text-sm md:text-base"><ChevronLeft className="w-4 h-4 md:w-5 md:h-5 mr-1" /> Previous</button>
                            <button disabled={currentQIndex === questions.length - 1} onClick={() => setCurrentQIndex(prev => prev + 1)} className="flex items-center text-blue-600 hover:text-blue-800 disabled:opacity-30 font-medium text-sm md:text-base">Next <ChevronRight className="w-4 h-4 md:w-5 md:h-5 ml-1" /></button>
                        </div>
                    </div>
                </div>
            </main>
  ...
  // 5. RESULT SCREEN
  if (view === 'result') {
    const totalPossibleScore = questions.length * settings.marksPerQuestion;
    const isPass = (score/totalPossibleScore) * 100 >= 50;
      ...
      <div className="min-h-screen bg-gray-50 p-4 font-sans text-gray-900 overflow-y-auto">
        <div className="max-w-3xl mx-auto space-y-6">
          <div className="bg-white p-6 md:p-8 rounded-2xl shadow-xl text-center">
            <div className="mb-4 flex justify-center">{isPass ? <CheckCircle className="w-12 h-12 text-green-500" /> : <XCircle className="w-12 h-12 text-red-500" />}</div>
            <h2 className="text-2xl md:text-3xl font-bold text-gray-900 mb-2">Test Completed</h2>
            <div className="mb-6 inline-block bg-gray-100 px-4 py-2 rounded-lg"><p className="text-[10px] md:text-sm text-gray-500 font-bold uppercase tracking-wide">Student ID</p><p className="text-lg md:text-xl font-mono font-bold text-gray-800">{guestId}</p></div>
            <div className="flex flex-col md:flex-row justify-center items-center gap-4 mb-8">
               <div className="text-center bg-gray-50 p-4 rounded-xl w-full md:w-1/3"><span className="block text-xs text-gray-400 uppercase font-bold tracking-wider">Score</span><span className={`text-3xl font-extrabold ${isPass ? 'text-green-600' : 'text-red-600'}`}>{score} / {totalPossibleScore}</span></div>
            </div>
            <button onClick={() => setView('welcome')} className="bg-gray-900 text-white px-6 py-3 rounded-xl font-bold w-full md:w-auto">Back to Home</button>
          </div>
          {/* Detailed Review */}
          <div className="bg-white rounded-2xl shadow-sm border border-gray-200 overflow-hidden">
             <div className="p-4 bg-gray-50 border-b border-gray-200 font-bold text-gray-700">Detailed Review</div>
             <div className="divide-y divide-gray-100">
                {questions.map((q, idx) => {
                    const userAns = answers[q.id];
                    const isCorrect = userAns === q.correctAnswer;
                    return (
                        <div key={q.id || idx} className="p-4 md:p-6">
                                ...
                                <div className="flex-1 min-w-0">
                                    ...
                                    <div className="flex flex-col gap-2 text-xs md:text-sm">
                                        <div className={`flex items-start p-2 rounded ${isCorrect ? 'bg-green-50 text-green-800' : 'bg-red-50 text-red-800'}`}>
                                            ...
                                            <span className="break-words">{!userAns ? "Skipped" : <span><span className="font-bold uppercase mr-1">{userAns.replace('option','')}</span> {q[userAns]}</span>}</span>
                                        </div>
                                        {!isCorrect && <div className="flex items-start p-2 rounded bg-green-50 text-green-800"><span className="font-bold w-20 md:w-24 flex-shrink-0 uppercase text-[10px] md:text-xs mt-0.5">Correct:</span><span className="break-words"><span className="font-bold uppercase mr-1">{q.correctAnswer.replace('option','')}</span>{q[q.correctAnswer]}</span></div>}
                                    </div>
                                </div>
                            </div>
                        </div>
