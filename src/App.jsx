...
export default function App() {
  ...
  const [view, setView] = useState(() => {
    ...
    // B. Check if a result is waiting in session
  ...
  // Ref to track programmatic exit
  ...
  // --- CRITICAL FIX: SELF-HEALING HISTORY GUARD ---
  // If we land on 'test' or 'result' but the data is missing (because we cleared it),
  // we must automatically bounce back or reset to welcome.
  useEffect(() => {
    if (view === 'test') {
      const savedEndTime = localStorage.getItem('cbt_endTime');
      // If no end time, this is a stale history entry (user pressed back into a dead test)
      if (!savedEndTime) {
        // Option A: If we want to skip this entry in history
      ...
      const savedResult = sessionStorage.getItem('cbt_currentResult');
      // If no result, this is a stale history entry
      if (!savedResult) {
         if (window.history.length > 1) {
             window.history.back();
         } else {
  ...
  useEffect(() => {
    ...
    if (view === 'test' || view === 'result') {
      // Only push trap if not already trapped to avoid duplicate stack entries on refresh
  ...
  const handleSubmit = () => {
    ...
    // Push a new clean state for result view so we can trap it
    window.history.pushState({}, '', window.location.href);
    setView('result');
  };

  // --- CRITICAL FIX: SMART EXIT HANDLER ---
  const handleExitToWelcome = (fromBackPress = false) => {
    ...
    // 2. Unwind History
    if (fromBackPress) {
        // User hit back button physically. Browser history is already updated.
        setView('welcome');
    } else {
        // User clicked "Home" button.
        // We cannot rely on go(-2) because of refreshes.
        // STRATEGY: Replace the current "Result/Test" entry with a neutral state
        // effectively "Deleting" the trap from the history.
        window.history.replaceState(null, '', window.location.href);
        setView('welcome');
    }

    // 3. Reset flag after animation/render frame
    setTimeout(() => {
                ...
                <div className="p-4 border-t border-gray-200 bg-gray-50 text-xs font-medium space-y-2">
                    ...
                    <div className="flex justify-between items-center mt-2">
                         <span className="flex items-center text-gray-600">
                             <div className="w-8 h-8 rounded-lg border bg-gray-50 ring-2 ring-blue-600 ring-offset-1 flex items-center justify-center font-bold text-xs text-blue-600 mr-2">
                                   {currentQIndex + 1}
                             </div>
                             Current
                         </span>
  ...
  // ---------------- RENDER: RESULT ----------------
  if (view === 'result') {
    // If no result data but we are in result view, the effect above will handle the redirect.
    // We can render null safely here to avoid crashes before the redirect happens.
    const resultDataStr = sessionStorage.getItem('cbt_currentResult');
    if (!resultDataStr) return null;
