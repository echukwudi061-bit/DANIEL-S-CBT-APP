...
export default function App() {
  ...
  // --- CRITICAL FIX 1: ROBUST HISTORY MANAGEMENT ---
  // Fixes: Duplicate history entries on refresh AND Trap logic
  useEffect(() => {
    const handlePopState = (event) => {
      ...
      // SCENARIO B: RESULT VIEW OR WELCOME VIEW (Double Tap)
      else if (view === 'result' || view === 'welcome') {
        const now = Date.now();
        ...
        if (timeDiff < 2000) {
          ...
          // If view is 'welcome', we simply allow the back event (exit app)
        } else {
    ...
    // UPDATE: Initialize trap for 'welcome' as well
    if (view === 'test' || view === 'result' || view === 'welcome') {
      // ESTABLISH TRAP: Only push if we haven't already trapped this specific state.
      // This prevents the stack from growing infinitely on Refresh.
  ...
  const handleSubmit = () => {
    ...
    // This allows the "Go Back -2" logic to work correctly vs "Go Back -1"
    window.history.pushState({}, '', window.location.href);
    setView('result');
  };

  // --- CRITICAL FIX 2: SMART EXIT LOGIC ---
  const handleExitToWelcome = (fromBackPress = false) => {
    ...
    // 3. Unwind Browser History "Smartly"
    if (fromBackPress) {
        // SCENARIO: User pressed the Phone/Browser Back button.
        // The browser has *already* popped the current Trap state.
        
        if (view === 'result') {
            // Stack was: [Welcome, Test, Result(Trap)]
            // User popped Result.
            // We are physically at 'Test'.
            // We need to pop 'Test' to get to 'Welcome'.
            window.history.back();
        } 
        else if (view === 'test') {
            // Stack was: [Welcome, Test(Trap)]
            // User popped Test.
            // We are physically at 'Welcome'.
            // No action needed.
        }
    } else {
        // SCENARIO: User clicked a UI button (Home / Quit).
        // The history stack is still fully intact. We must pop manually.
        if (view === 'result') {
            // Stack: [Welcome, Test, Result] -> Go back 2 steps
            window.history.go(-2);
        } else if (view === 'test') {
            // Stack: [Welcome, Test] -> Go back 1 step
            window.history.back();
        }
    }

    // 4. Fallback & View Reset
    setTimeout(() => {
        setView('welcome');
  ...
  if (view === 'welcome') {
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center p-4 font-sans text-gray-900">
        {/* ADDED: Toast component to Welcome Screen */}
        ...
        <div className="flex flex-1 overflow-hidden relative">
            ...
            `}>
                ...
                <div className="p-4 border-t border-gray-200 bg-gray-50 text-xs font-medium space-y-2">
                    ...
                    {/* UI REQUIREMENT: SIDEBAR LEGEND FIXED */}
                    <div className="flex justify-between items-center mt-2">
                         <span className="flex items-center text-gray-600">
                             {/* Replaced large square with small hollow ring */}
