...
// Reusable component to ensure consistent styling across all views
...
export default function App() {
  ...
  // MODIFIED: 'view' now persists from localStorage so refresh keeps you on the same screen
  ...
  const [guestId, setGuestId] = useState(() => {
    // We try to keep the guest ID per session/browser load. 
    // If you want a new one every refresh, remove the localStorage check.
    // However, keeping it in localStorage prevents it from changing if the user accidentally refreshes during a test.
    const saved = localStorage.getItem('cbt_guestId');
    if (saved) return saved;
    
    // Generate new 4 digit random ID
    const randomNum = Math.floor(1000 + Math.random() * 9000); // 1000 to 9999
    return `GUEST ID: ${randomNum}`;
  });

  // --- Test Taking State (Now Persisted) ---
  // MODIFIED: All test states (index, answers, score, timer) now initialize from localStorage
  ...
  const [timeLeft, setTimeLeft] = useState(() => {
    const saved = localStorage.getItem('cbt_timeLeft');
    return saved ? parseInt(saved, 10) : 0;
  });
  ...
  useEffect(() => { localStorage.setItem('cbt_timeLeft', timeLeft.toString()); }, [timeLeft]);
  ...
  const handleSubmit = () => {
    ...
    const newResult = {
      id: Date.now(),
      guestId: guestId, // Store the guest ID with the result
      score: finalScore,
      total: totalPossible,
                   ...
                   {testResults.length === 0 ? (
                     <tr>
                       ...
                       <tr key={res.id} className="hover:bg-gray-50">
                         {/* GUEST ID IN ADMIN TABLE - BLUE COLOR */}
    ...
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center p-4 font-sans text-gray-900">
        <div className="bg-white p-8 rounded-2xl shadow-xl max-w-md w-full text-center border border-blue-100 relative">
          ...
          {/* GUEST ID: Top Right Corner of Main Card */}
        ...
        <header className="bg-white shadow-sm px-3 md:px-6 py-3 flex justify-between items-center z-20 flex-shrink-0 w-full relative">
          <div className="flex items-center gap-2 overflow-hidden mr-2">
            ...
            <div className="flex flex-col overflow-hidden min-w-0 items-start">
              ...
              {/* GUEST ID: Top Left Corner Below Title */}
              <GuestIdBadge id={guestId} />
            </div>
          </div>
